    ...
    <nav style={{ position:'sticky', top:0, zIndex:100, background:'white', borderBottom:'1px solid #e8e2d9', padding:'0 1.5rem' }}>
      <div style={{ maxWidth:1160, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', height:68 }}>
        ...
        <ul style={{ display:'flex', gap:'1.5rem', alignItems:'center', listStyle:'none', margin:0, padding:0 }} className="rb-desktop-nav">
          ...
          <li>
            <Link href="https://outcome-ready.vercel.app" target="_blank" style={{ fontSize:'0.85rem', color:'#2d5a3d', fontWeight:700, textDecoration:'none', border:'1.5px solid #4a7c59', borderRadius:20, padding:'0.3rem 0.8rem', display:'flex', alignItems:'center', gap:5 }}>
              <span style={{ fontSize:'0.7rem' }}>🌿</span> OutcomeReady
            </Link>
          </li>
          <li>
        ...
        <div style={{ background:'white', borderTop:'1px solid #e8e2d9', padding:'1rem 1.5rem' }}>
          {[
            ...
            <Link key={href} href={href} onClick={() => setOpen(false)}
              style={{ display:'block', padding:'0.6rem 0', fontSize:'0.95rem', color:'#1a1a1a', textDecoration:'none', borderBottom:'1px solid #f0ebe3' }}>
              {label}
            </Link>
          ))}
          <Link href="https://outcome-ready.vercel.app" target="_blank" onClick={() => setOpen(false)}
